...
⚠️ УСТАРЕЛО (Спринт 2): функции fill_inv_ru()/fill_pl() заменены единым
конвейером canonical → app/engines/sheet_writer.py + app/engines/fillers/optiauto.py
(одна механика на INV и PL, корректный сдвиг формул через Translator, выбор
шаблона по отправителю). Сервис их больше НЕ вызывает. Модуль оставлен только
ради переиспользуемых хелперов `_parse_date` и `country_to_iso2`.
